...
app.controller("nc", function ($location, $anchorScroll, $scope) {
	...
	$scope.updateSwitchAddress = function(machine, netkit, interfaceIndex) {
		var selectedDeviceName = machine.interfaces.if[interfaceIndex].selectedDevice;
		var selectedDevice = netkit.find(device => device.name === selectedDeviceName);
		
		if (selectedDevice) {
			// Lấy số eth hiện tại dựa trên interfaceIndex
			var ethNumber = interfaceIndex; // giả sử thiết bị đầu tiên tương ứng với eth0, thiết bị thứ hai với eth1, v.v.
			
			// Tạo chuỗi eth (ví dụ: eth0, eth1, ...)
			var ethString = 'eth' + ethNumber;
			
			// Giả sử selectedDevice có một mảng giao diện và bạn muốn giao diện đầu tiên
			var selectedInterface = selectedDevice.interfaces.if[0]; 
			
			// Cập nhật giao diện máy với eth và ip của thiết bị đã chọn
			machine.interfaces.if[interfaceIndex].eth = {
				number: ethNumber,
				domain: selectedInterface.eth.domain // giả sử bạn muốn miền từ thiết bị đã chọn
			};
			machine.interfaces.if[interfaceIndex].ip = selectedInterface.ip; 
			
			// Cập nhật các biến trong scope để hiển thị
			$scope.selectedDeviceEth = selectedInterface.eth.domain;
			$scope.selectedDeviceIp = selectedInterface.ip;
		} else {
			// Nếu thiết bị không được tìm thấy, bạn có thể xóa thông tin giao diện
			machine.interfaces.if[interfaceIndex].eth = {}; // Hoặc giữ nguyên giá trị cũ
			machine.interfaces.if[interfaceIndex].ip = ''; // Hoặc giữ nguyên giá trị cũ
		}
	};
	$scope.addInterface = function(machine) {
		var newInterface = {
			eth: {
				number: machine.interfaces.counter,
				domain: ''
			},
			macAddress: '',  // Thêm trường macAddress cho interface
			ip: '',
			name: '',
			selectedDevice: ''
		};
		machine.interfaces.if.push(newInterface);
		machine.interfaces.counter++;
	};
	
	$scope.removeInterface = function(machine) {
		if (machine.interfaces.if.length > 1) {
			machine.interfaces.if.pop();
			machine.interfaces.counter--;
		}
	};
	
	// Cập nhật thông tin switch và các thiết bị kết nối
	$scope.updateSwitchAddress = function(machine, netkit, index) {
		var interfaceData = machine.interfaces.if[index];
		// Cập nhật địa chỉ MAC hoặc thông tin liên quan đến switch nếu cần thiết
		console.log('Updated MAC Address: ' + interfaceData.macAddress);
	};
