...
//  ----------SWITCH-------------
// function makeSwitch(netkit, lab) {
//     for (let machine of netkit) {
//         // Kiểm tra xem máy có tên và loại là 'switch_network' không
//         if (machine.name && machine.name !== "" && machine.type === "switch_network") {
//             // Kiểm tra xem máy có được kích hoạt không
//             if (machine.switch && machine.switch.enable) {
//                 // Thêm thư mục cho bộ chuyển mạch nếu cần (có thể loại bỏ nếu không cần thiết)
//                 lab.folders.push(`${machine.name}/config`);

//                 // Tạo file cấu hình cho bộ chuyển mạch
//                 lab.file[`${machine.name}/config/${machine.name}.cfg`] = `
//                     # Cấu hình bộ chuyển mạch ${machine.name}
//                     interface eth0
//                     description Kết nối đến Terminal hoặc Router
//                     no shutdown
//                 `;

//                 // Tạo file startup cho PC tương ứng với switch
//                 let startupFileContent = "";
//                 for (let interface of machine.interfaces.if) {
//                     if (interface.ip && interface.ip !== "") {
//                         startupFileContent += `ifconfig eth${interface.eth.number} ${interface.ip.split('/')[0]} up\n`;
//                         startupFileContent += `ifconfig eth${interface.eth.number} hw ether 00:00:00:00:00:${("0" + interface.eth.number).slice(-2)}\n`;
//                     }
//                 }

//                 // Lưu nội dung vào lab.file
//                 lab.file[`${machine.name}.startup`] = startupFileContent;

//                 // Nếu có kết nối với terminal hoặc router, thêm cấu hình tương ứng
//                 if (machine.connections && machine.connections.length > 0) {
//                     for (let connection of machine.connections) {
//                         lab.file[`${machine.name}/config/${machine.name}.cfg`] += `
//                             interface ${connection.interface}
//                             switchport access vlan ${connection.vlan}
//                             description Kết nối đến ${connection.device}
//                         `;
//                     }
//                 }
//             }
...
// 	for (let machine of netkit) {
// 	  if (machine.type === "pc") {
// 		// Generate script for PC
// 		scriptContent += `# Configure PC ${machine.name}\n`;
// 		scriptContent += `echo "PC configuration" > ${machine.name}.script\n`;
// 	  } else if (machine.type === "router") {
// 		// Generate script for router
// 		scriptContent += `# Configure router ${machine.name}\n`;
// 		scriptContent += `echo "Router configuration" > ${machine.name}.script\n`;
// 	  }
// 	}
    ...
    for (let machine of netkit) {
        ...
        if (machine.name && machine.name !== "" && machine.type === "switch") {
            ...
            if (machine.switch && machine.switch.enable) {
                ...
                // Tạo file startup cho switch
                let startupFileContent = "";

                // Thêm lệnh tạo cầu nối và thêm các giao diện vào cầu nối
                startupFileContent += `brctl addbr br0\n`;
                
                // Thêm các giao diện vào cầu nối
                for (let interface of machine.interfaces.if) {
                    if (interface.eth.number !== 0) { // Chỉ thêm các giao diện không phải eth0
                        startupFileContent += `brctl addif br0 eth${interface.eth.number}\n`;
                    }
                }

                // Kích hoạt cầu nối và STP
                startupFileContent += `brctl stp br0 on\n`;
                startupFileContent += `ifconfig br0 up\n`;
...
// 					}
// 				}
// 				if (machine.type !== "switch_network" && machine.type !== "router") {
...
// 						lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} hw ether 00:00:00:00:00:${("0" + interface.eth.number).slice(-2)}\n`;
...
// 				if (machine.type === "switch_network") {
// 					lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} up\n`;
// 					lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} hw ether 00:00:00:00:00:${("0" + interface.eth.number).slice(-2)}\n`;
// 					lab.file[machine.name + ".startup"] += `brctl addbr br0\n`;
// 					lab.file[machine.name + ".startup"] += `brctl addif br0 eth${interface.eth.number}\n`; // Sửa từ 'addbr' thành 'addif'
// 					lab.file[machine.name + ".startup"] += `brctl stp br0 on\n`;
// 					lab.file[machine.name + ".startup"] += `ifconfig br0 up\n`; 
// 				}
// 			}
...
// 					}
// 					else {
// 						lab.file[machine.name + ".startup"] += "route add -net " + gateway.route +" gw "+gateway.gw+ "\n";
// 					}
// 				}
// 			}
...
// function makeStaticRouting(netkit, lab) {
//     let switchCounter = 1; // Đếm cho các switch
//     let bridgeCounter = 0; // Đếm cho các bridge
...
// 				if (machine.type !== "switch_network" && machine.type !== "router") {
// 					if (interface.eth.domain && interface.eth.domain !== "" && interface.ip && interface.ip !== "") {
// 						// Trích xuất số thứ hai từ địa chỉ IP
// 						const ipParts = interface.ip.split('.');
// 						const secondOctet = ipParts[2]; // Lấy phần thứ hai của địa chỉ IP
				
// 						lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} ${interface.ip} up\n`;
// 						lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} hw ether 00:00:00:00:00:${("0" + secondOctet).slice(-2)}\n`;
// 						//lab.file[machine.name + ".startup"] += "ip address add "+interface.ip+" dev eth" + interface.eth.number+"\n";
// 					}
// 				}
...
// 				if (machine.type === "switch_network") {
// 					bridgeCounter++; // Tăng số lượng bridge
// 					const bridgeName = `br${bridgeCounter}`; // Tạo tên bridge mới
				
// 					// Tạo một mảng để lưu các eth đã được cấu hình
// 					const ethInterfaces = [];
				
// 					// Thực hiện vòng lặp qua các giao diện của máy
// 					for (let interface of machine.interfaces.if) {
// 						// Kiểm tra xem interface có địa chỉ IP không
// 						if (interface.ip && interface.ip !== "") {
// 							// Trích xuất số thứ hai từ địa chỉ IP
// 							const ipParts = interface.ip.split('.');
// 							const secondOctet = ipParts[2]; // Lấy phần thứ hai của địa chỉ IP
				
// 							// Cấu hình eth interfaces
// 							lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} up\n`;
// 							lab.file[machine.name + ".startup"] += `ifconfig eth${interface.eth.number} hw ether 00:00:00:00:${("0" + secondOctet).slice(-2)}:${("0" + secondOctet).slice(-2)}\n`;
							
// 							// Thêm eth vào mảng
// 							ethInterfaces.push(`eth${interface.eth.number}`);
// 						}
// 					}
				
// 					// Thêm cầu nối
// 					lab.file[machine.name + ".startup"] += `brctl addbr ${bridgeName}\n`;
				
// 					// Thêm tất cả các eth vào cầu nối
// 					for (let eth of ethInterfaces) {
// 						lab.file[machine.name + ".startup"] += `brctl addif ${bridgeName} ${eth}\n`; // Kết nối switch với bridge
// 					}
				
// 					lab.file[machine.name + ".startup"] += `brctl stp ${bridgeName} on\n`;
// 					lab.file[machine.name + ".startup"] += `ifconfig ${bridgeName} up\n`; // Kích hoạt bridge
// 				}
//             }
    ...
    for (let machine of netkit) {
        if (machine.name && machine.name != "") {
            for (let interface of machine.interfaces.if) {
                ...
                if (machine.type !== "switch" && machine.type !== "router") {
                    if (interface.eth.domain && interface.eth.domain !== "" && interface.ip && interface.ip !== "") {
                        // Trích xuất số thứ hai từ địa chỉ IP
                        const ipParts = interface.ip.split('.');
                        const secondOctet = ipParts[2]; // Lấy phần thứ hai của địa chỉ IP
            ...
            // Xử lý cho switch_network
            if (machine.type === "switch") {
                bridgeCounter++; // Tăng số lượng bridge
                const bridgeName = `br${bridgeCounter}`; // Tạo tên bridge mới
                const ethInterfaces = []; // Mảng lưu các eth đã cấu hình
                ...
                // Chỉ in cấu hình cho switch_network một lần
                lab.file[machine.name + ".startup"] += `brctl addbr ${bridgeName}\n`;
                for (let eth of ethInterfaces) {
                    lab.file[machine.name + ".startup"] += `brctl addif ${bridgeName} ${eth}\n`; // Kết nối switch với bridge
                }
